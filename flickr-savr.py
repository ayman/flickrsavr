...
class FlickrSavr(object):
    ...
    def get_photo(self, photo):
        ...
        # get image
        # TODO: This should exit with the photo number and then put
        # that in a second queue.
        try:
        ...
        try:
            ...
            pass
        pools = []
        # TODO: this call failed...maybe try to wrap all the bad ids
        ...
        metadata.set_tag_string("Xmp.flickrsavr.description",
                                photo['description']['_content'])
        metadata.set_tag_string("Xmp.flickrsavr.dateupload",
                                photo['dateupload'])
        metadata.set_tag_string("Xmp.flickrsavr.datetaken",
                                photo['datetaken'])
        metadata.set_tag_long("Xmp.flickrsavr.datetakengranularity",
                              int(photo['datetakengranularity']))
        metadata.set_tag_long("Xmp.flickrsavr.datetakenunknown",
                              int(photo['datetakenunknown']))
        metadata.set_tag_long("Xmp.flickrsavr.views", int(photo['views']))
        metadata.set_tag_string("Xmp.flickrsavr.machine_tags",
                                photo['machine_tags'])
        ...
        if ('latitude' in photo and 'longitude' in photo):
            metadata.set_tag_string(
                "Xmp.flickrsavr.latitude", str(photo['latitude']))
            metadata.set_tag_string(
                "Xmp.flickrsavr.longitude", str(photo['longitude']))
        if ('accuracy' in photo):
            metadata.set_tag_long(
                "Xmp.flickrsavr.accuracy", int(photo['accuracy']))
        if ('tags' in photo):
